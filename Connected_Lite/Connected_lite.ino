...
void Command_AP_STA_Config(String modem_mode_ssid_pswd)
{
  ...
  temp = "ESP_NODE=" +WiFi.macAddress()+ "=" + WiFi.softAPmacAddress()+ "=LITE_SENT=OK" ;
  webSocket.broadcastTXT(temp);
  temp = "";
  delay(1000);
      WiFi.disconnect();
...
// =================================================================================================
// Description: Handles all Lite commands. First check initial setup is working or not
// =================================================================================================
void Command_Lite(String Lite_data)
{
   int Number_Of_Token = 0;
    StringTokenizer tokens(Lite_data, "=");
    while(tokens.hasNext()){
        // prints the next token in the string
        LITE_CODE[Number_Of_Token] = tokens.nextToken();
#if DEBUG
         Serial.println(LITE_CODE[Number_Of_Token]);
#endif
        Number_Of_Token ++;   
    }
    Serial.println(LITE_CODE[1]);
    LITE_CODE[0]= ""; LITE_CODE[1]= "";//Make the string NULL
...
{   
    ...
    else
    {
      if(Parsed_data[2] == "CONF" || Parsed_data[2] == "CONT" || Parsed_data[2] == "STATUS")
      {
        ...
        else if(Parsed_data[3] == "777")
          Command_Lite(Parsed_data[4]);
